...
| 战略/任务 | 昼夜/返防/终盘/紧急风险近似，基础采售、动态建造环内三炮建设、开拓者接题；简化 MissionSpec 以建造格或武器 ID 作为稳定目标键，完整 spec 相同时跨步骤复用 mission/plan | 完整 MissionSpec 字段、任务 DAG、指令/预算、全队租约分配及完整任务状态转移 |
...
| `SpatialIndex` | 静态障碍、当前动态占用、交互邻域、由基地生成的建造区域、计划占位 |
...
`RuleSet` 集中提供距离、实体占地、能力、建造区域、成本、昼夜、弹道、物品效果和字段要求。运行时明确返回的射程、冷却、商品名称与价格优先用于当前决策，缺失信息不伪造为确认值。规则表带版本及来源。

用户已确认建造区几何：基地是以接口 `pos` 为左上角的 2×2；基地外扩一格形成 4×4，减去基地的 12 格只能建武器；再外扩一格形成 6×6，减去内层 4×4 的 20 格只能建围墙。规则层以观测基地位置动态生成区域并裁剪地图边界，换边无需写死坐标或实体 ID。建设规划仍须排除实际障碍、预约格，并验证基地出口及武器操控站位连通。
...
首阶段已把 `ColdStart → Ready`、同会话地图尺寸/阵营突变导致的 `Ready → Degraded`、以及一致新帧的 `Degraded → Ready` 接入 `runtime::Session`。降级不会替换最后有效快照、增加世界版本或复用旧缓存动作；恢复帧重新生成差分。`EventLog` 保存稳定递增 ID、来源回合和类型化事件，容量满时移除最旧记录；`EventInbox` 已提供按层游标、确认和截断检测。关键未消费证据的保留/重建策略、实例路由和显式 `Closed` 仍是后续实现，当前简化日志不能替代完整目标 inbox。
...
| 状态 | 进入时 | 每回合行为与输出 | 退出时 |
| --- | --- | --- | --- |
| `Bootstrap` | 建立阵营、能力和规则可用性清单 | 确认当前阶段、基地及已有单位；若无存活基地则禁建，但不阻塞其他初始化 | 发布第一版战略指令 |
...
当前代码的 `EventInbox` 为 Strategy/Mission/Tactics/Individual 分别保存“下一个待消费 ID”。`poll` 返回有序批次及 `truncated`，`acknowledge` 仅接受与当前起点一致的 receipt；任一读者确认不会推进其他读者。战略读者属于 `DecisionState` 草稿，只有整轮编码与截止检查成功后才随草稿提交，因此失败规划不会吞掉事件。战略层消费 `WorldRecovered` 后清零紧急退出的连续安全证据，再从恢复帧重新累计；其他 guard 仍以完整观测重验。当前批次只有 `WorldEvent`，尚无目标信封中的 phase、route、filter、priority 和关键事件溢出恢复。
...
当前垂直切片按 reporter 和简化 `MissionSpec { kind, objective }` 保存 assignment。经济循环与挑战会话使用固定目标键，建设使用建造格，守备用武器 ID；完整 spec 相同的后续动作复用 mission/plan 并只创建新 intent。提案接受后才撤销前一 intent；kind 或 objective 改变时撤销旧 mission/plan；本地拒绝只失效候选节点。仲裁接收及编译响应时均调用 `ActiveOwners::is_current`。`PendingAction` 和下一帧 `ExecutionReport` 复用动作路径，旧路径报告保留事实但不能推进替代工作。当前 spec 尚无 goal、依赖、期限、优先级和租约；持久任务 DAG 建成后，应由完整 MissionAssignment 提供 owner。
...
- 布防以武器可覆盖区域、控制者可达性、墙体保护与出入口连通性评分；只搜索规则层生成的对应建造环。覆盖已有武器需显式替换任务授权，防止意外降回 level1。
...
| 编号 | 问题与来源 | 当前处理 | 验证办法 |
| --- | --- | --- | --- |
| U01（已解决） | 用户确认基地 2×2、武器为周围 4×4 减基地、围墙为周围 6×6 减 4×4 | `rules/build.rs` 从观测基地动态生成并按地图裁剪；武器/围墙严格分区 | 单元测试验证 12/20 格和边界；正式联调继续核对裁判接受结果 |
