...
经济循环与挑战会话使用固定目标键，建设使用建造格，守备用武器 ID；只有完整 spec 相同的后续动作才复用 mission/plan 并创建新 intent。自动相对期限一经分配会复用冻结值，显式 deadline、priority 或其他策略变化仍会建立新 assignment。提案先用当前观测角色检查 required capabilities：工人提供 Gather/Sell/Build/OperateWeapon，开拓者提供 SolveChallenge/OperateWeapon；缺失、死亡、生命未知或缺少任一能力均在创建 owner 前拒绝。攻击检查 controller，协议 actor 仍是武器。RetryExhausted 失败保留完整 spec、失败回合和尝试次数；同一 assignee 的精确失败契约在 owner 分配前被墓碑抑制，契约或规则窗口改变后可创建新实例。新任务获准后才取消旧任务及依赖后代，取消结果同时失效 ActiveOwners 父链。仲裁接收及编译响应时均调用 `ActiveOwners::is_current`，`PendingAction` 和下一帧 `ExecutionReport` 复用动作路径。当前自动策略仍未创建依赖；租约、其他任务 checkpoint/progress、MissionFactory、全队 AssignmentSolver 和 ResourceCoordinator 仍待实现。
