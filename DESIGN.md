...
| 世界/事件 | `ColdStart/Ready/Degraded` 生命周期、有效快照保留/恢复、带稳定 ID 的有界事件日志、实体差分、敌人记忆及两格任务点合并 | `Closed` 会话信号、三类知识和完整实体索引、预测/来源、按订阅者确认的 inbox 与报告路由 |
| 战略/任务 | 昼夜/返防/终盘/紧急风险近似，基础采售、动态建造环内三炮建设、开拓者接题 | 指令/预算、全队租约分配、完整任务 DAG 与状态转移 |
...
首阶段已把 `ColdStart → Ready`、同会话地图尺寸/阵营突变导致的 `Ready → Degraded`、以及一致新帧的 `Degraded → Ready` 接入 `runtime::Session`。降级不会替换最后有效快照、增加世界版本或复用旧缓存动作；恢复帧重新生成差分。`EventLog` 保存稳定递增 ID、来源回合和类型化事件，容量满时移除最旧诊断记录。按层订阅者游标、未消费证据保护和显式 `Closed` 仍是后续实现，当前日志不能替代完整 inbox。
