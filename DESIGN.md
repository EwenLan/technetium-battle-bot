...
> 更新：2026-09-19。状态：设计基线，尚未实现。当前代码仍为 Rust 2024 的 `Hello, world!`。
...
以下目录为目标结构，除 `src/main.rs`、`Cargo.toml` 和已有文档外均待创建。先采用单 Cargo package 与内部模块，不提前拆 workspace。
...
tools/                     # 回放和比赛结果分析工具
run.sh                     # 未来比赛入口：bash run.sh <port>
```
...
拟采用 `serde/serde_json` 处理协议、`axum/tokio` 处理 HTTP、`tracing` 记录日志；版本、MSRV 和比赛构建环境在 P0/P1 阶段确认后锁定。本设计不代表依赖已经安装。
