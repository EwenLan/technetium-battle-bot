...
| 战略/任务 | 昼夜/返防/终盘/紧急风险近似，基础采售、动态建造环内三炮建设、开拓者接题；四类工作族的角色 assignment 跨步骤复用 mission/plan | MissionSpec/目标键、任务 DAG、指令/预算、全队租约分配及完整任务状态转移 |
...
当前垂直切片按 reporter 和 Construction/Economy/Challenge/Defense 工作族保存简化 assignment，同类后续动作复用 mission/plan 并只创建新 intent。提案接受后才撤销前一 intent；工作族改变时撤销旧 mission/plan；本地拒绝只失效候选节点。仲裁接收及编译响应时均调用 `ActiveOwners::is_current`。`PendingAction` 和下一帧 `ExecutionReport` 复用动作路径，旧路径报告保留事实但不能推进替代工作。该工作族键不等于正式任务目标；持久任务 DAG 建成后，应由带目标键、期限和租约的 MissionAssignment 提供 owner。
