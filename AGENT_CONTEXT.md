...
U01 已解决：用户确认基地 2×2，武器环为周围 4×4 减基地，围墙环为周围 6×6 减内层 4×4。待确认统一编号：U02 回合起点/复活边界、U03 任务交互区域、U04 HTTP/空字段契约、U05 示例字段差异、U06 弹道/伤害细节、U07 会话与结果关联、U08 操炮动作槽、U09 编译运行环境、U10 挑战 timeout 起算点。完整描述及降级策略只在 [DESIGN.md](DESIGN.md) 第 12 节维护。
...
### 2026-09-20：任务目标证据与期限终态闭环

- 新增 GoalEvaluation/GoalEvidence：建设从指定位置的存活己方建筑取证，挑战与防守分别使用 ChallengeEnded、进入白昼事件，AllOf 合并去重；证据保留实体/事件 ID 和来源回合。
- MissionRegistry 每轮在决策草稿中先求值目标，再执行 deadline 守卫；Succeeded 保存完成证据并解锁依赖，Expired 保留根终态并将依赖后代 Cancelled，DecisionState 同步失效相关 OwnerPath。
...
### 2026-09-21：实时任务规则窗口期限

- 新增 deadline 生成器：经济/建设使用当日最后 ActionSubmission 回合，挑战再取 timeout 与答案余量边界，防守使用下一白昼首回合的 EffectObservation 边界。
