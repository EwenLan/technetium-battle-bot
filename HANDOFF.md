...
`domain::owner` 已实现类型化 mission/plan/intent ID、generation、确定性 `OwnerAllocator`、`OwnerPath` 与 `ActiveOwners`。任务/战术动作先在决策草稿中生成带完整路径的 `ActionProposal`；`command::Arbiter` 在接收提案和编译响应前各校验一次完整父链。初次校验失败会撤销候选路径并保留角色原工作，最终失效的提案不会进入响应或等待记录。攻击 actor 是武器、reporter 是操控角色；等待和 Step 报告复用提案 owner。旧动作仍产生事实报告，但失效父链不能改变替代工作的简化 FSM。合法回执不等于实际效果。

当前 owner 仍按每个候选动作创建，尚未来自持久任务 assignment；简化 `ActionProposal` 只有 actor、owner、action，未包含正式契约的 ProposalId、TurnStamp、claims、预期效果和原子组。任务层仍缺 `StrategicDirective`、任务 DAG/租约。战略已有阶段选择，个体已有提交动作的简化等待/回执处理；其余 enum 不代表完成的状态机。仲裁目前允许 `move/collect/sell/build/attack/acceptTask/submitAnswer/use Medicine` 的受限子集。夜间火力是基础贪心，无穿透/溅射结算模型和联合配对。
...
最新阶段已通过 rustfmt、Clippy warnings-as-errors、47 个 Rust 测试（2 个单元测试、45 个集成测试）、locked release 构建和本机 HTTP 冒烟。OwnerPath/ActionProposal 验证覆盖本地拒绝保留原工作、失效 owner 不编码、攻击归属控制者，以及“旧回执保留但不推进替代工作”；该结果不代表完整 P2 或官方判题器联调完成。
