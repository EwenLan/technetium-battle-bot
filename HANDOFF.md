...
`domain::owner` 已实现类型化 mission/plan/intent ID、generation、确定性 `OwnerAllocator`、`OwnerPath` 与 `ActiveOwners`。仲裁选择的动作才会在决策草稿中获得完整路径并登记为 `WaitingResult`；下一新回合按 `lastRoundRoleActionResults` 生成携带同一 owner 和具体 intent scope 的 Step 报告。攻击 actor 是武器、reporter 是操控角色。替代工作会撤销角色旧 mission；旧动作仍产生事实报告，但完整父链失效检查阻止它改变新工作的简化 FSM。合法回执不等于实际效果。

当前 owner 仍按实际提交动作创建，尚未来自持久任务 assignment，也未进入仲裁前的 `ActionProposal`；任务层仍缺 `StrategicDirective`、任务 DAG/租约。战略已有阶段选择，个体已有提交动作的简化等待/回执处理；其余 enum 不代表完成的状态机。仲裁目前允许 `move/collect/sell/build/attack/acceptTask/submitAnswer/use Medicine` 的受限子集。夜间火力是基础贪心，无穿透/溅射结算模型和联合配对。
...
最新阶段已通过 rustfmt、Clippy warnings-as-errors、44 个集成测试、locked release 构建和本机 HTTP 冒烟。OwnerPath 新增验证覆盖确定分配和“旧回执保留但不推进替代工作”；该结果不代表完整 P2 或官方判题器联调完成。
