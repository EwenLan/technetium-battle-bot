...
`domain::owner` 已实现类型化 mission/plan/intent ID、generation、确定性 `OwnerAllocator`、`OwnerPath` 与 `ActiveOwners`。mission/plan/intent 注册保留 active 状态和代次墓碑，父层或单层失效都会被完整链校验识别。角色以简化 `MissionSpec { kind, objective }` 维护 assignment：经济和挑战使用固定会话目标，建设按建造格、守备按武器 ID 区分目标。完整 spec 相同时复用 mission/plan，每个动作提案创建新 intent；目标变化时替换 assignment。`command::Arbiter` 在接收提案和编译响应前各校验一次完整父链，等待和 Step 报告复用提案 owner。

当前 `MissionSpec` 只是过渡值对象：assignment 另存 owner，spec 尚无 goal、依赖、期限、优先级、租约或任务 DAG；经济和挑战的固定目标也未区分具体矿点、商店或挑战代次。简化 `ActionProposal` 只有 actor、owner、action，未包含正式契约的 ProposalId、TurnStamp、claims、预期效果和原子组。战略已有阶段选择，个体已有提交动作的简化等待/回执处理；其余 enum 不代表完成的状态机。仲裁目前允许 `move/collect/sell/build/attack/acceptTask/submitAnswer/use Medicine` 的受限子集。夜间火力是基础贪心，无穿透/溅射结算模型和联合配对。
...
最新阶段已通过 rustfmt、Clippy warnings-as-errors、57 个 Rust 测试（6 个单元测试、51 个集成测试）、locked release 构建和本机 HTTP 冒烟。OwnerPath/assignment 验证覆盖相同目标跨步骤复用、同类目标切换、旧 intent 失效、代次墓碑、草稿回滚、本地拒绝保留原工作及旧回执隔离；该结果不代表完整 P2 或官方判题器联调完成。
